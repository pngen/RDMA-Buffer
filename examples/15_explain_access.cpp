...
int main() {
    ...
    q.authority = exutil::envelope(); q.domain = dom; q.backend = syn->id(); q.node = NodeId(1); q.process = ProcessId(1);
    AccessDecision ad = rt.validate_remote_access(q);
    std::printf("%s\n", explain_remote_access(q, ad).c_str());
    std::printf("%s\n", explain_backend_capability(syn->capabilities(), MemoryDomain::SYNTHETIC_REMOTE_CAPABLE,
                  access_mask(AccessRight::REMOTE_WRITE)).c_str());
    return 0; }
