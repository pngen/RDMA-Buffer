...
int main() {
    ...
    // Cross-domain access is rejected.
    ...
    q.authority = exutil::envelope(); q.domain = ProtectionDomainId(999); q.backend = syn->id(); q.node = NodeId(1); q.process = ProcessId(1);
    AccessDecision ad = rt.validate_remote_access(q);
    std::printf("cross-domain access=%s explanation=%s\n", ad.code.c_str(), ad.explanation.c_str());
    return 0; }
